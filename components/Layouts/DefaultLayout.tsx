...
import ConnectBoxed from '@/pages/auth/boxed-connect';
...
const DefaultLayout = ({ children }: PropsWithChildren) => {
    ...
    const [showTopButton] = useState(false);
    ...
    useEffect(() => {
        if (session_status === 'loading' || status === 'connecting') {
            setShowLoader(true)
        } else {
            setShowLoader(false)
        }
    ...
    useEffect(() => {
        if (isDisconnected && session_status === 'authenticated') {
    ...
    return (
        <App>
            ...
            <div className="relative">
                ...
                {/*<div className={`${(!config.sidebar && 'hidden') || ''} fixed inset-0 z-50 bg-[black]/60 lg:hidden`} onClick={() => dispatch(toggleSidebar())}></div>*/}
                ...
                <div className={`${config.navbar} main-container min-h-screen text-black dark:text-white-dark`}>
                    {/* BEGIN SIDEBAR */}
                    <Sidebar />
                    {/* END SIDEBAR */}
                    <div className="main-content flex flex-col min-h-screen lg:w-[calc(100%-70px)] lg:ml-[70px]">
                        {/* BEGIN TOP NAVBAR */}
                        <Header />
                        {/* END TOP NAVBAR */}

                        {/* BEGIN CONTENT AREA */}
                        {isConnected && session_status === 'authenticated' && chain && (
                            <div className={`${config.animation} animate__animated p-6`}>{children}</div>
                        )}
